...
export default function HistoryArchive() {
    ...
    // Fetch workspace projects for filter
    useEffect(() => {
        async function fetchWorkspace() {
            try {
                const res = await fetch('/api/dashboard/workspace');
                const data = await res.json();
                // Find current org and set its projects
                const org = data.find((o: any) => o.slug === slug);
                if (org) setProjects(org.projects || []);
            } catch (err) {
    ...
    useEffect(() => {
        fetchArchive();
    }, [page, projectId, startDate, endDate]);
            ...
            <div className="max-w-[1400px] mx-auto">
                ...
                <div className="mb-8 flex justify-between items-center">
                    <div>
                        <div className="flex items-center gap-2 text-gray-400 mb-1">
                            <History size={16} />
                            <span className="text-xs font-bold uppercase tracking-widest">History Archive</span>
                        </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-6 flex flex-wrap gap-4 items-end">
                    <div className="flex-1 min-w-[200px]">
                        <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1.5 tracking-wider">Project</label>
                        <select
                            value={projectId}
                    ...
                    <button
                        onClick={() => { setProjectId(''); setStartDate(''); setEndDate(''); setPage(1); }}
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50/50 border-b border-gray-100">
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider w-32">Project</th>
                                ...
                                {loading ? (
                                    ...
                                    incidents.map((incident) => (
                                        <tr key={incident.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className="text-xs font-bold text-gray-600 bg-gray-100 px-2 py-0.5 rounded uppercase tracking-wider">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <button
                                                    onClick={() => router.push(`/org/${slug}/proj/${incident.projectId}/history/${incident.id}`)}
